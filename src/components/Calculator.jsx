...
const Calculator = ({ answers, digits, possibles, handleValidAnswer }) => {
  ...
  const inputHandler = (input, id = -1) => {
    const { newInputArr, newDigitsUsed, newCursorPos } = handleInput(
      [...inputArr],
      [...digitsUsed],
      input,
      cursorPos,
  ...
  const backspaceHandler = () => {
    ...
    const newInputArr = [
      ...inputArr.slice(0, cursorPos - 1),
      ...inputArr.slice(cursorPos),
    ];

    let digitsIndex = 9;
    for (let i = 0; i <= digits.length; i += 1) {
      if (digits[i] === deletable && digitsUsed.includes(i)) {
        digitsIndex = i;
      }
    }
    if (deletable === "1" && digitsUsed.includes(-1)) {
      digitsIndex = -1;
    }
  ...
  const checkAnswer = () => {
    ...
    if (digitsUsed.length !== 4 || digitsUsed.includes(-1)) {
      return "Use each given digit once";
    }
    if (!areBracketsBalanced(inputArr.join(""))) {
      return "Check your brackets";
    }
  ...
  return (
    <div className="grid">
      <div className="output span-four">
        ...
        {isAllFound() && <div className="output-calculation">CONGRATS!</div>}
      ...
      {operations.map((input, id) => {
        return (
          <InputButton key={id} input={input} clickHandler={inputHandler} />
        );
      })}
      <div
      ...
      <div
        className="button span-four equals"
        role="button"
        tabIndex="0"
